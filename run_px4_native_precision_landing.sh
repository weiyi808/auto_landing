#!/usr/bin/env bash
# PX4 v1.12 原生精准降落完整启动器
#
# 直接完整运行（启用 PX4 landing-target 输出，飞行员仍手动解锁和切模式）：
#   ./run_px4_native_precision_landing.sh
# 地面测试（完整启动传感器和定位，但不创建 landing-target 发布器）：
...
# px4_pos_estimator -> /mavros/vision_pose/pose -> PX4 本地位置 -> landing target。
...
OTHER_SESSION_NAME="landing_companion"
...
usage() {
  cat <<'EOF'
用法：
  ./run_px4_native_precision_landing.sh          # 完整运行，启用 PX4 目标输出
  ./run_px4_native_precision_landing.sh test     # 完整测试链路，禁用目标输出
  ./run_px4_native_precision_landing.sh status   # 查看会话、定位和目标状态
  ./run_px4_native_precision_landing.sh stop     # 仅停止本脚本创建的会话
EOF
}
...
require_package() { rospack find "$1" >/dev/null 2>&1 || die "缺少 ROS 包：$1"; }
source_environment() {
  set +u
  source "$ROS_SETUP"
...
node_exists() { rosnode list 2>/dev/null | grep -Fxq "$1"; }
deadline_reached() { (( $(date +%s) >= $1 )); }
...
wait_for_native_publisher() {
  local deadline=$(( $(date +%s) + STARTUP_TIMEOUT_S ))
  if [[ "$DRY_RUN" == "1" ]]; then
    return 0
  fi
  until rostopic info /mavros/landing_target/pose 2>/dev/null | \
      grep -Fq '/px4_landing_target'; do
    deadline_reached "$deadline" && return 1
    sleep 0.5
  done
}
...
show_status() {
  ...
  if ! session_exists "$SESSION_NAME"; then
    echo "[PX4 原生降落] 会话未运行：$SESSION_NAME"
    return 1
  fi
  source_environment
  echo "[PX4 原生降落] tmux 窗口"
  tmux list-windows -t "$SESSION_NAME"
  echo "[PX4 原生降落] 飞控状态"
  timeout 3 rostopic echo -n 1 /mavros/state || true
  echo "[PX4 原生降落] landing-target 坐标系"
  rosparam get /mavros/landing_target/mav_frame || true
  ...
  echo "[PX4 原生降落] 节点状态"
  timeout 3 rostopic echo -n 1 /precision_landing/px4_native/status || true
  echo "[PX4 原生降落] landing-target 发布者"
  rostopic info /mavros/landing_target/pose || true
  echo "[PX4 原生降落] BODY_NED 输出发布者（原生节点不应发布）"
...
stop_session() {
  require_command tmux
  if ! session_exists "$SESSION_NAME"; then
    echo "[PX4 原生降落] 会话已经停止：$SESSION_NAME"
    return 0
  fi
  local pane pane_pid
  ...
  echo "[PX4 原生降落] 已停止会话：$SESSION_NAME"
}
...
enable_target_publish="true"
[[ "$mode" == "test" ]] && enable_target_publish="false"
...
session_exists "$OTHER_SESSION_NAME" && die "伴随降落方案正在运行：$OTHER_SESSION_NAME"
...
tmux new-session -d -s "$SESSION_NAME" -n overview \
  "bash -lc 'echo PX4 原生精准降落启动会话; exec bash'"
if ! rosparam get /run_id >/dev/null 2>&1; then
...
wait_for_topic_message /camera/color/image_raw || die "D455 彩色图像超时，请查看 $SESSION_NAME:d455"
wait_for_topic_message /camera/color/camera_info || die "D455 CameraInfo 超时，请查看 $SESSION_NAME:d455"

start_window fastlio "roslaunch fast_lio mapping_mid360andmaping.launch"
wait_for_topic_message /drone_Odometry || die "FAST-LIO /drone_Odometry 超时，请查看 $SESSION_NAME:fastlio"

start_window vision "roslaunch fly_utils px4_pos_estimator.launch"
wait_for_topic_message /mavros/vision_pose/pose || die "视觉位置传输超时，请查看 $SESSION_NAME:vision"
wait_for_topic_message /mavros/local_position/pose || \
  die "PX4 本地位置超时，请检查 EKF2 外部视觉配置和 $SESSION_NAME:vision"

mav_frame="$(rosparam get /mavros/landing_target/mav_frame 2>/dev/null || true)"
if [[ "$enable_target_publish" == "true" && "$mav_frame" != "LOCAL_NED" ]]; then
  die "直接运行要求 /mavros/landing_target/mav_frame=LOCAL_NED，当前为：${mav_frame:-未设置}"
elif [[ "$mav_frame" != "LOCAL_NED" ]]; then
  echo "[PX4 原生降落] test 模式提示：landing-target 坐标系不是 LOCAL_NED，输出保持禁用。"
fi

if [[ "$enable_target_publish" == "true" ]]; then
  echo "[PX4 原生降落] 请确保已在 QGroundControl/PX4 控制台确认 PX4 v1.12 的 PLD_* 参数和 precision-land 支持。"
fi
start_window landing "roslaunch precision_landing d455_px4_native_precision_landing.launch enable_target_publish:=$enable_target_publish board_scale:=$BOARD_SCALE"
wait_for_topic_message /precision_landing/px4_native/status || \
  die "PX4 原生降落节点状态超时，请查看 $SESSION_NAME:landing"
if [[ "$enable_target_publish" == "true" ]]; then
  wait_for_native_publisher || die "landing-target 发布器启动超时，请查看 $SESSION_NAME:landing"
fi

start_window monitor "watch -n 1 'rostopic echo -n 1 /mavros/state; rostopic echo -n 1 /precision_landing/px4_native/status'"

echo "[PX4 原生降落] 完整链路已启动，enable_target_publish=$enable_target_publish"
echo "[PX4 原生降落] 查看窗口：tmux attach -t $SESSION_NAME"
if [[ "$mode" == "test" ]]; then
  echo "[PX4 原生降落] 当前为 test 模式，不发布 landing target。"
else
  echo "[PX4 原生降落] 目标输出已允许；解锁和飞行模式切换仍由飞行员手动完成。"
fi
